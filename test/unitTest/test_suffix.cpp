        ...
        TEST_F (SuffixUnitTest, checkEqualityTest) {
            ...
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 5; j++) {
                    ...
                    if (i == 0)
                        suffixes_ = new BitvectorSuffix(builder_->getSuffixType(), suffix_len, 0,
                                                        builder_->getSuffixes(), num_suffix_bits_per_level, 0, height);
                    else if (i == 1)
                        suffixes_ = new BitvectorSuffix(builder_->getSuffixType(), 0, suffix_len,
                                                        builder_->getSuffixes(), num_suffix_bits_per_level, 0, height);
                    else
                        suffixes_ = new BitvectorSuffix(builder_->getSuffixType(), suffix_len, suffix_len,
                                                        builder_->getSuffixes(), num_suffix_bits_per_level, 0, height);
