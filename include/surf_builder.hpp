...
namespace surf {
    ...
    public:
        SuRFBuilder() : sparse_start_level_(0), suffix_type_(kNone) {
            values_ = std::make_shared<std::vector<std::vector<Value>>>(0);
        };
        ...
        void build(const std::vector<std::pair<std::vector<label_t>,Value>> &keys) {
            assert(keys.size() > 0);
            buildSparse(keys);
            if (include_dense_) {
        ...
        std::shared_ptr<std::vector<std::vector<Value>>> getValues() const {
            return values_;
        }
    ...
    private:
        ...
        void buildSparse(const std::vector<std::pair<std::vector<label_t>,Value>> &keys) {
        ...
        level_t
        insertKeyBytesToTrieUntilUnique(const std::vector<label_t> &key, const Value value, const std::vector<label_t> &next_key, const level_t start_level) {
